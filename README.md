...
| 同じ課題に既にコメント済み    | 「投稿済み」                                 | Backlog `/api/v2/users/myself` で取得した自分の Backlog ID が、`/api/v2/issues/{key}/comments` の `createdUser.id` に含まれていれば弾く（最大 5 ページ＝500 コメントまで遡る）。Backlog 側でコメントを削除すれば再度投稿可能。 |
